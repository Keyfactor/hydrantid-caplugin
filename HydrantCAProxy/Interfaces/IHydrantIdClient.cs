...
{
    public interface IHydrantIdClient
    {
        Task<CertRequestResult> GetSubmitEnrollmentAsync(CertRequestBody registerRequest);
        Task<CertRequestResult> GetSubmitRenewalAsync(string certificateId, RenewalRequest renewRequest);
        Task<List<Policy>> GetPolicyList();
        ...
        Task GetSubmitCertificateListRequestAsync(BlockingCollection<ICertificatesResponseItem> bc, CancellationToken ct);
        Task<bool> Ping();
    }
}
